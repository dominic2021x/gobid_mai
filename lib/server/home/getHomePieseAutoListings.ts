/**
 * Server-only: recent marketplace listings for subcategory „Piese auto” (homepage strip).
 */
...
async function fetchHomePieseAutoListingsUncached(): Promise<HomePremiumItem[]> {
  ...
  const { data: productsData, error } = await runPostgrestQuery<Record<string, unknown>[]>(
    (signal) =>
      admin
        ...
        .eq("status", "active")
        .eq("category", PIESE_AUTO_CATEGORY_SLUG)
        .eq("subcategory", PIESE_AUTO_SUBCATEGORY_SLUG)
        .neq("product_type", "licitatii-publice")
...
export async function getHomePieseAutoListings(): Promise<HomePremiumItem[]> {
  return unstable_cache(
    fetchHomePieseAutoListingsUncached,
    ["home-piese-auto-listings-v1"],
    { revalidate: 60, tags: ["home-piese-auto", "ro-listings"] }
  )();
}
