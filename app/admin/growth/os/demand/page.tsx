...
interface StatusData {
  snapshot: { opportunities?: unknown[]; generatedAt?: string } | null;
...
export default function DemandPage() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/admin/growth/os/demand/status", {
      });
  ...
  const handleRefresh = useCallback(async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/admin/growth/os/demand/enqueue", {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
  ...
  const handleCreateCandidates = useCallback(async () => {
    const token = await getAdminToken();
    if (!token) return;
    setCreateLoading(true);
    setMessage(null);
    try {
      const res = await fetch("/api/admin/growth/os/demand/create-candidates", {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) setMessage({ type: "error", text: (json?.error as string) ?? `Error ${res.status}` });
      else {
        setMessage({ type: "success", text: `Create candidates enqueued (${json.jobId ?? "ok"}). Run worker.` });
        setTimeout(fetchStatus, 2000);
      }
    } catch (e) {
      setMessage({ type: "error", text: e instanceof Error ? e.message : "Request failed" });
  ...
  const handleStatusPatch = useCallback(
    async (id: string, newStatus: "accepted" | "ignored") => {
      ...
      setPatchingId(id);
      try {
        const res = await fetch(`/api/admin/growth/os/demand/${encodeURIComponent(id)}`, {
          method: "PATCH",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Search Demand Mining</h1>
          <p className="mt-1 text-sm text-slate-500">
            Internal + GSC + suggestions → ranked opportunities. Create LP candidates.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            {refreshLoading ? "Enqueuing…" : "Refresh mining"}
          </button>
          <button
            type="button"
          ...
          >
            {createLoading ? "Enqueuing…" : "Create candidates"}
          </button>
        </div>
      </div>
      {message && (
      ...
      <Card title="Top opportunities" description="query, score, intent, county, category, action, status" accent="blue">
        {loading ? (
          <p className="text-sm text-slate-500">Loading…</p>
        ) : opportunities.length === 0 ? (
          <p className="text-sm text-slate-500">No opportunities. Run Refresh mining.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-left">
                  ...
                  <th className="p-2 font-medium text-slate-700">action</th>
              ...
              <tbody>
                {opportunities.map((o) => (
                  <tr key={o.id} className="border-b border-slate-100">
                    ...
                    <td className="p-2 text-slate-600">{o.demand_score}</td>
                    ...
                    <td className="p-2 text-slate-600">{o.recommended_action}</td>
                    <td className="p-2">
                      <span
                        className={
                          "rounded px-1.5 py-0.5 text-xs " +
                          (o.status === "done" ? "bg-emerald-100 text-emerald-800" : o.status === "accepted" ? "bg-blue-100 text-blue-800" : o.status === "ignored" ? "bg-slate-100 text-slate-600" : "bg-amber-100 text-amber-800")
                        }
                      >
                        {o.status}
                      </span>
