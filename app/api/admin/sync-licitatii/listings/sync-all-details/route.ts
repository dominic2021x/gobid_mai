/**
 * POST – actualizează TOATE câmpurile (ca butonul "Toate câmpurile" dar în bulk).
 * Body: { ids?: string[] } – dacă ids e prezent și nevid, doar acele anunțuri; altfel toate active (max 3000).
 * Header x-details-stream: 1 → răspuns NDJSON cu log live (type: "log" per anunț, type: "done" la final).
 */
...
export async function POST(request: NextRequest) {
  ...
  const useStream = request.headers.get("x-details-stream") === "1";
  ...
  if (toProcess.length === 0) {
    return NextResponse.json({
      ...
      failed: 0,
      results: [],
      message: ids?.length ? "Niciun anunț valid pentru ID-urile selectate." : "Niciun anunț de procesat.",
    });
  }
      ...
      async start(controller) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
          ...
          try {
            ...
            const { update: fullUpdate, imageUrls } = buildDetailUpdatePayload(detail);
            ...
            const modifiedFields = Object.keys(updatePayload).filter((k) => k !== "updated_at");
            ...
            if (updateError) {
              ...
              send({ type: "log", index, total, source_external_id: listing.source_external_id, success: false, error: updateError.message });
              continue;
            }
            ...
            if (existingImages?.length) {
              await db.from("licitatii_insolventa_listing_images").delete().eq("listing_id", listing.id);
              imagesUpdated = true;
            }
            if (imageUrls.length > 0) {
              const imageRows = imageUrls.map((url, idx) => ({ listing_id: listing.id, url, sort_order: idx }));
  ...
  return NextResponse.json({
    ...
    failed,
    results,
    message: `Procesate ${toProcess.length}: ${updated} cu toate câmpurile actualizate, ${failed} eșecuri.`,
  });
}
