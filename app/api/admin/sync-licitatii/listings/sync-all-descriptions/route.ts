/**
 * POST – actualizează doar descrierile (description_html) pentru anunțuri.
 * Body: { ids?: string[] } – dacă ids e prezent și nevid, se procesează doar acele anunțuri; altfel toate active (max 1000).
 * Procesare la rând (unul câte unul). Returnează results[] cu numerotare și eroare per anunț.
 */
...
/** PostgREST/Supabase limită numărul de parametri în .in(); împărțim în batch-uri. */
...
export async function POST(request: NextRequest) {
  ...
  if (ids && ids.length > 0) {
    ...
    if (onlyMissing) {
      query = query.is("description_html", null);
    }
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      if (updateError) {
        ...
        const length = detail.descriptionHtml ? detail.descriptionHtml.length : 0;
        results.push({
          index,
          id: listing.id,
          source_external_id: listing.source_external_id,
  ...
  return NextResponse.json({
    ...
    failed,
    results,
    message: `Procesate ${toProcess.length}: ${updated} actualizate, ${failed} eșecuri.`,
  });
}
