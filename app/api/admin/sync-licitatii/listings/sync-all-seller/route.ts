/**
 * POST – actualizează doar câmpurile vânzător (nume, email, telefon, adresă) pentru anunțuri.
 * Body: { ids?: string[] } – ids: doar acele anunțuri; altfel toate active (max 3000).
 * Header x-seller-stream: 1 → răspuns NDJSON cu log live (type: "log" per anunț, type: "done" la final).
 */
...
const SELLER_GROUP: DetailUpdateGroup = "seller";
...
function pickSellerUpdate(fullUpdate: Record<string, unknown>): Record<string, unknown> {
  const keys = DETAIL_UPDATE_GROUPS[SELLER_GROUP];
  const out: Record<string, unknown> = {};
  for (const k of keys) {
    if (k in fullUpdate) out[k] = fullUpdate[k] === undefined ? null : fullUpdate[k];
  }
...
export async function POST(request: NextRequest) {
  ...
  if (ids && ids.length > 0) {
    ...
    if (onlyMissing) {
      query = query.or(
        "seller_name.is.null,seller_name.eq.,seller_email.is.null,seller_email.eq.,seller_phone.is.null,seller_phone.eq.,seller_address.is.null,seller_address.eq."
      );
    }
  ...
  if (toProcess.length === 0) {
    return NextResponse.json({
      ...
      failed: 0,
      results: [],
      message: ids?.length ? "Niciun anunț valid pentru ID-urile selectate." : onlyMissing ? "Niciun anunț fără detalii vânzător complete." : "Niciun anunț de procesat.",
    });
  }

  const useStream = request.headers.get("x-seller-stream") === "1";
      ...
      async start(controller) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
          ...
          try {
            ...
            const payload = pickSellerUpdate(fullUpdate);
  ...
  return NextResponse.json({
    ...
    failed,
    results,
    message: `Procesate ${toProcess.length}: ${updated} detalii vânzător actualizate, ${failed} eșecuri.`,
  });
}
