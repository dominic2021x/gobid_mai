/**
 * POST – actualizează doar locația (location_raw, location_city, location_county) pentru anunțuri.
 * Body: { ids?: string[], onlyMissing?: boolean } – ids: doar acele anunțuri; altfel toate active (max 1000). onlyMissing: doar cele fără județ.
 * Header x-counties-stream: 1 → răspuns NDJSON cu log live (type: "log" per anunț, type: "done" la final).
...
import { normalizeLocation } from "@/lib/scraper/location";
...
export async function POST(request: NextRequest) {
  ...
  if (ids && ids.length > 0) {
    ...
    if (onlyMissing) {
      query = query.or("location_county.is.null,location_county.eq.");
    }
  ...
  if (toProcess.length === 0) {
    return NextResponse.json({
      ...
      failed: 0,
      results: [],
      message: ids?.length ? "Niciun anunț valid pentru ID-urile selectate." : onlyMissing ? "Niciun anunț fără județ." : "Niciun anunț de procesat.",
    });
  }

  const useStream = request.headers.get("x-counties-stream") === "1";
      ...
      async start(controller) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
          ...
          try {
            ...
            const loc = detail.locationRaw ? normalizeLocation(detail.locationRaw) : { raw: "", city: null, county: null };
            const location_raw = detail.locationRaw ?? null;
            const location_city = loc.city ?? null;
            const location_county = loc.county ?? null;
  ...
  return NextResponse.json({
    ...
    failed,
    results,
    message: `Procesate ${toProcess.length}: ${updated} județe/orase actualizate, ${failed} eșecuri.`,
  });
}
