/**
 * IPN Netopia / mobilPay pentru cumpărare tokeni
 */
  ...
  try {
    ...
    if (contentType.includes('application/x-www-form-urlencoded')) {
      ...
      if (validation.success && validation.action) {
        ...
        if (orderId && orderId.startsWith('TKN-')) {
          await fulfillTokenPurchaseByIntentId(supabaseAdmin, orderId);
        }
      }
    ...
    if ((paymentStatus === 3 || paymentStatus === 5) && orderId && orderId.startsWith('TKN-')) {
      await fulfillTokenPurchaseByIntentId(supabaseAdmin, orderId);
    }
