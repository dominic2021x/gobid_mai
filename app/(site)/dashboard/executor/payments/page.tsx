...
export default function ExecutorPaymentsPage() {
  ...
  // Load credits directly from Supabase via API route (bypasses RLS)
  const loadCreditsFromSupabase = async () => {
    try {
      ...
      if (!creditsResponse.ok) {
        ...
        let errorData: any = {};
        const responseText = await creditsResponse.text();
        
        try {
          errorData = JSON.parse(responseText);
        } catch (e) {
          errorData = { message: responseText || 'Unknown error', rawResponse: responseText };
        }
        setUserCredits(0);
        setInvoices([]);
        setTransactions([]);
  ...
  const loadPaymentsFromSupabase = async (providedSession?: any, providedUserId?: string | null) => {
    try {
      // Use provided session/userId or get from Supabase
      ...
      // If session exists but no access token, try to refresh
      ...
      if (!userId) {
        userId = session?.user?.id || (typeof window !== "undefined" ? localStorage.getItem('supabaseUserId') : null);
      }
  ...
  const handleCreditPurchase = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'netopia') {
      try {
        ...
        setMessage({ type: 'error', text: data.error || 'Eroare la redirecționarea către plată. Încearcă din nou.' });
      } catch (err) {
  ...
  const approveCreditTransaction = (transactionId: string) => {
  ...
  // Real invoices data - loaded from localStorage
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (typeof window !== "undefined") {
          if (pathname?.startsWith("/dashboard/lichidator") && accountType === "executor") {
            window.location.href = "/dashboard/executor/payments";
            return;
          }
          if (pathname?.startsWith("/dashboard/executor") && accountType === "liquidator") {
            window.location.href = "/dashboard/lichidator/payments";
            return;
          }
        }
        ...
        // Load credits and payments from Supabase via API route
        // This ensures we get the latest data from Supabase
        // Only call if we have both session and userId
        if (sessionData.session && userId) {
          await loadPaymentsFromSupabase(sessionData.session, userId);
        }
      } catch (error) {
    ...
    const loadTokensFromSupabase = async () => {
      try {
        ...
        // Error loading tokens from Supabase
      }
    ...
    // Page loading is already handled by loadPaymentsFromSupabase
    setIsPageLoading(false);
  }, []);

  // Update credit balance when invoices change (fallback calculation)
  // Only update if API hasn't set a credit value yet (to avoid overriding API data)
  useEffect(() => {
    // Only recalculate if we have invoices and credit is still 0 (API hasn't loaded yet)
    if (invoices.length > 0 && userCredits === 0) {
      const creditFromInvoices = calculateCreditBalance();
      if (creditFromInvoices > 0) {
        setUserCredits(creditFromInvoices);
        localStorage.setItem('userCredits', creditFromInvoices.toString());
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [invoices]);

  // Listen for Supabase realtime changes on user_payments table (polling below is fallback)
  useEffect(() => {
    const warnRt = warnOnceOnRealtimeFailure(
      "ExecutorPayments",
      "user_payments",
      "Aceeași publicație Realtime ca la /dashboard/payments (migrație 20260127)."
    );
    ...
    const setupRealtime = async () => {
      ...
      const channel = supabase
        .channel('executor_payments_changes', {
          config: {
            broadcast: { self: true },
            presence: { key: userId }
          }
        ...
        .on(
          ...
          (_payload: RealtimePostgresChangesPayload<Record<string, unknown>>) => {
            loadPaymentsFromSupabase(session, userId);
          }
        )
        .subscribe((status: string) => {
          if (status === "SUBSCRIBED" && process.env.NODE_ENV === "development") {
            console.log("[ExecutorPayments] Realtime: user_payments subscribed");
          }
          warnRt(status);
        });
  ...
  // Refresh credits from Supabase periodically (realtime updates)
  useEffect(() => {
    const refreshData = async () => {
      try {
        ...
        // Refresh credits and payments directly from Supabase (realtime)
        await loadPaymentsFromSupabase(session, userId);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      const paymentsKey = `payments_${userEmail}`;
      const invoicesKey = `invoices_${userEmail}`;
      
      // If payments or invoices changed, reload them
      if (e.key === paymentsKey || e.key === invoicesKey || e.key === 'invoices') {
        // Reload invoices/payments
        const savedPayments = localStorage.getItem(paymentsKey);
        const savedInvoices = localStorage.getItem(invoicesKey) || localStorage.getItem('invoices');
        ...
        if (savedInvoices) {
          try {
            ...
            if (Array.isArray(parsedInvoices)) {
              ...
              const newInvoices = parsedInvoices.filter((inv: any) => !existingIds.has(inv.id));
              allPayments = [...allPayments, ...newInvoices];
            }
          } catch (e) {
            // Error parsing invoices
          }
        }
        
        if (allPayments.length > 0) {
          setInvoices(allPayments);
        }
      }
    };

    // Also listen for same-origin storage events
    ...
    const handleCustomStorage = () => {
      const paymentsKey = `payments_${userEmail}`;
      const savedPayments = localStorage.getItem(paymentsKey);
      const savedInvoices = localStorage.getItem(`invoices_${userEmail}`) || localStorage.getItem('invoices');
    ...
    const intervalId = setInterval(handleCustomStorage, 2000);
  ...
  const handleDownloadLocalInvoice = async (invoice: any) => {
    ...
    // Template Oblio: verde profesional
    const oblioGreen = [0, 166, 81]; // #00a651
    ...
    if (config) {
      try {
        ...
        if (smartbillConfig.username && smartbillConfig.token && smartbillConfig.companyVATNumber) {
          ...
          // SmartBill configured but full integration requires API route. Using local template.
        }
      } catch (error) {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-300 relative ${
      ...
      <div className="container mx-auto max-w-7xl px-2 sm:px-4 py-4 sm:py-8 flex-1 relative z-10">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold mb-2 ${
                    ...
                    Centrul de Plăți - Executor
                  </h2>
                </div>
              </div>
              
              {/* Credit Balance */}
              <div className={`mt-4 md:mt-0 backdrop-blur-sm rounded-xl p-4 border ${
                isDarkMode 
                  ? 'bg-white/5 border-white/10' 
                  : 'bg-white/10 border-gray-200/30'
              }`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-2xl font-bold ${
                      ...
                      {userCredits > 0 ? `${userCredits.toLocaleString('ro-RO')} Lei` : '0 Lei'}
                    </p>
                  </div>
                  {!isIosNativeApp && (
                    <button
                      onClick={() => setShowCreditModal(true)}
                      className="ml-4 px-4 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg hover:from-yellow-600 hover:to-yellow-700 transition-all duration-300 text-sm font-medium shadow-lg hover:shadow-xl transform hover:scale-105"
                    >
                      + Cumpără Credit
                    </button>
                  )}
        ...
        {isIosNativeApp && (
          <div className="mb-6">
            <div className={`backdrop-blur-sm rounded-2xl p-6 shadow-2xl border ${
              isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/10 border-gray-200/30'
            }`}>
              <AppleCreditBundles
                isDarkMode={isDarkMode}
          ...
          <div className={`mb-6 p-4 rounded-lg backdrop-blur-sm shadow-xl border ${
            message.type === 'success'
              ? isDarkMode
                ...
                : 'bg-green-50/50 text-green-800 border-green-200/50'
              : isDarkMode
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            {/* Balance Card */}
              <div className={`backdrop-blur-sm rounded-2xl p-6 shadow-2xl border transition-all duration-300 ${
                isDarkMode 
                  ...
                  : 'bg-white/10 border-gray-200/30 hover:bg-white/20'
              }`}>
              <div className="flex items-center justify-between">
                <div>
                    <p className={`text-sm font-medium ${
                    ...
                    <p className={`text-2xl font-bold ${
                      isDarkMode ? 'text-white' : 'text-gray-900'
                    }`}>{calculateCreditBalance().toLocaleString('ro-RO')} Lei</p>
                </div>
                  <div className={`p-3 backdrop-blur-sm rounded-xl border ${
                    ...
                    <ArrowUpIcon size="m" className={isDarkMode ? "text-green-400" : "text-green-600"} />
                </div>
              </div>
            </div>
            ...
            {/* Active Cards */}
              <div className={`hidden md:block backdrop-blur-sm rounded-2xl p-6 shadow-2xl border transition-all duration-300 ${
                isDarkMode 
                  ? 'bg-white/5 border-white/10 hover:bg-white/10' 
                  : 'bg-white/10 border-gray-200/30 hover:bg-white/20'
              }`}>
                  ...
                  <tbody>
                    {transactions.length === 0 ? (
                      <tr>
                        ...
                        >
                          Nu există tranzacții încă. După plăți, istoricul apare din datele contului.
                        </td>
                      </tr>
                    ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {invoices.map((invoice) => (
                <div key={invoice.id} className={`backdrop-blur-sm rounded-2xl p-6 shadow-2xl border-l-4 border-[#00a651] transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-white/5 border-white/10 hover:bg-white/10' 
                    : 'bg-white/10 border-gray-200/30 hover:bg-white/20'
                }`}>
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <p className={`text-2xl font-bold ${
                      ...
                      {(invoice.total || invoice.amount || 0).toLocaleString('ro-RO')} {displayCurrencyLabel(invoice.currency)}
                    </p>
                    <p className={`text-sm ${
                      isDarkMode ? 'text-gray-400' : 'text-gray-500'
                    }`}>
                      Data: {invoice.date || 'N/A'}
                    </p>
                    <p className={`text-sm ${
                      isDarkMode ? 'text-gray-400' : 'text-gray-500'
                    }`}>
                      Scadentă: {invoice.dueDate || 'N/A'}
                    </p>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-[#00a651] text-white py-2 rounded-lg hover:bg-[#047857] transition-all duration-300 text-sm font-medium shadow-lg hover:shadow-xl transform hover:scale-105"
                  >
                    Vezi Detalii
                  </button>
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        ...
                        className="flex-1 bg-[#00a651] text-white py-2 px-3 rounded-lg hover:bg-[#047857] transition-all duration-300 text-sm flex items-center justify-center shadow-lg hover:shadow-xl transform hover:scale-105"
                      >
                        <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        {oblioStatus.enabled ? 'PDF (Oblio)' : 'PDF'}
                      </button>
                      {oblioStatus.enabled && (
                        <button 
                          onClick={() => handleDownloadLocalInvoice(invoice)}
                          className="flex-1 bg-emerald-700 text-white py-2 px-3 rounded-lg hover:bg-emerald-800 transition-all duration-300 text-sm flex items-center justify-center shadow-lg hover:shadow-xl"
                          title="Descarcă template local gobid.ro"
                        >
                          PDF (local)
                        </button>
                      )}
                      <button 
                        ...
                        className="flex-1 bg-emerald-700 text-white py-2 px-3 rounded-lg hover:bg-emerald-800 transition-all duration-300 text-sm flex items-center justify-center shadow-lg hover:shadow-xl transform hover:scale-105"
                      >
                        <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {activeTab === 'cards' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {savedCards.map((card) => (
                <div key={card.id} className={`backdrop-blur-sm rounded-xl p-4 shadow-xl border transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-white/5 border-white/10 hover:bg-white/10' 
                    : 'bg-white/10 border-gray-200/30 hover:bg-white/20'
                }`}>
      ...
      {/* Payment Modal */}
      {showPaymentModal && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-2 md:p-4 bg-black/10 dark:bg-black/15">
          <div className={`backdrop-blur-md rounded-2xl p-8 shadow-2xl border max-w-md w-full max-h-[95vh] overflow-y-auto ${
            isDarkMode 
      ...
      {showInvoiceModal && selectedInvoice && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-2 md:p-4 bg-black/10 dark:bg-black/15">
          <div className={`rounded-xl md:rounded-2xl shadow-2xl max-w-4xl w-full transition-all overflow-hidden max-h-[95vh] overflow-y-auto backdrop-blur-md ${isDarkMode ? 'bg-white/10 border border-white/20' : 'bg-white/30 border border-gray-200/50'}`}>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-emerald-100 text-xs md:text-sm">
                    Emisă pe {selectedInvoice.date || selectedInvoice.created_at ? new Date(selectedInvoice.date || selectedInvoice.created_at).toLocaleDateString('ro-RO') : 'N/A'}
                  </p>
                </div>
              <button 
            ...
            <div className="p-4 md:p-8 space-y-4 md:space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                <div className={`p-4 md:p-6 rounded-xl border-l-4 border-[#00a651] backdrop-blur-sm ${isDarkMode ? 'bg-white/5 border border-white/10' : 'bg-white/10 border border-gray-200/30'}`}>
                ...
                <div className={`p-4 md:p-6 rounded-xl border-l-4 border-emerald-600 backdrop-blur-sm ${isDarkMode ? 'bg-white/5 border border-white/10' : 'bg-white/10 border border-gray-200/30'}`}>
                  <div className="flex items-center mb-3 md:mb-4">
                  ...
                  <div className={`text-xs md:text-sm transition-colors ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                    <p className="font-medium text-sm md:text-base mb-1">
                      {selectedInvoice.buyer?.name || selectedInvoice.user_name || userInfo.firstName && userInfo.lastName ? `${userInfo.firstName} ${userInfo.lastName}` : 'Utilizator'}
                    </p>
                    <p>{selectedInvoice.buyer?.email || selectedInvoice.user_email || userInfo.email || 'N/A'}</p>
                    <p>{selectedInvoice.buyer?.address || selectedInvoice.user_address || 'N/A'}</p>
                  </div>
                </div>
              </div>
                    ...
                    <tbody>
                      {selectedInvoice.items && Array.isArray(selectedInvoice.items) && selectedInvoice.items.length > 0 ? (
                        selectedInvoice.items.map((item: any, index: number) => (
                          <tr key={index} className={`border-t transition-colors hover:${isDarkMode ? 'bg-gray-750' : 'bg-gray-50'} ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                            <td className={`py-2 md:py-4 px-2 md:px-6 transition-colors ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                              <div className="font-medium text-xs md:text-sm">{item.name || item.description || 'Item'}</div>
                            </td>
                            ...
                            <td className={`text-right py-2 md:py-4 px-2 md:px-6 transition-colors text-xs md:text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                              {(item.price || item.amount || 0).toLocaleString()} Lei
                            </td>
                            <td className={`text-right py-2 md:py-4 px-2 md:px-6 font-semibold text-xs md:text-sm transition-colors ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                              {((item.price || item.amount || 0) * (item.quantity || 1)).toLocaleString()} Lei
                            </td>
                          </tr>
                        ))
                      ) : (
                        // Fallback: show single payment as invoice item
                        <tr className={`border-t transition-colors ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                          <td className={`py-2 md:py-4 px-2 md:px-6 transition-colors ${isDarkMode ? 'text-white' : 'text-gray-900'}`} colSpan={4}>
                            <div className="font-medium text-xs md:text-sm">
                              {selectedInvoice.description || selectedInvoice.payment_type || 'Plată'}
                            </div>
                            {selectedInvoice.invoice_number && (
                              <div className="text-xs text-gray-500 mt-1">
                                Factură: {selectedInvoice.invoice_number}
                              </div>
                            )}
                          </td>
                        </tr>
              ...
              <div className={`p-4 md:p-6 rounded-xl md:rounded-2xl ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
                <div className="flex flex-col space-y-4">
                  <div className="space-y-2">
                    ...
                    Data: {selectedInvoice.date || selectedInvoice.created_at ? new Date(selectedInvoice.date || selectedInvoice.created_at).toLocaleDateString('ro-RO') : 'N/A'}
                  </p>
                    </div>
                    <div className="flex items-center">
                      <svg className={`w-4 h-4 md:w-5 md:h-5 mr-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {selectedInvoice.dueDate ? (
                        <p className={`text-xs md:text-sm transition-colors ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                          Scadentă: {new Date(selectedInvoice.dueDate).toLocaleDateString('ro-RO')}
                        </p>
                      ) : null}
                </div>
                  </div>
                  <div className="flex flex-col space-y-3">
                    <div className="flex items-center justify-center">
                      <span className={`px-3 md:px-4 py-2 rounded-full text-xs md:text-sm font-medium border ${getStatusColor(selectedInvoice.status || 'completed')}`}>
                  {getStatusText(selectedInvoice.status || 'completed')}
                </span>
              </div>
                    <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-3">
                      ...
                      <button 
                        ...
                        className="flex-1 bg-emerald-700 text-white py-2 px-4 rounded-lg hover:bg-emerald-800 transition-colors font-medium text-sm md:text-base flex items-center justify-center"
                      >
                        <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                        </svg>
