...
// @ts-nocheck - Fișier alternativ/WIP; ruta folosește page.tsx. Elimină ts-nocheck după ce completezi variabilele lipsă (showQuickAddModal, showCounterOfferModalChat, etc.).
...
export default function ExecutorMyProductsPage() {
  ...
  const [counterOfferAmountChat, setCounterOfferAmountChat] = useState('');

  // ========== MODAL MANUAL ADD - STATE-URI ==========
  ...
  // Helper functions pentru modal manual
  ...
  // Load user city from profile when Manual Add modal opens (optional logic)
  useEffect(() => {
    if (!showManualAddModal) return;
    // Optional: load user city when modal opens (add logic here if needed)
  }, [showManualAddModal]);
  ...
  const handleSaveLicitator = async () => {
    ...
    try {
      const updateData: any = {
        ...
        licitator_fiscal_code: licitatorData.licitatorFiscalCode || null,
        licitator_consignment_account: licitatorData.licitatorConsignmentAccount || null,
        licitator_email: licitatorData.licitatorEmail || null,
        licitator_phone: licitatorData.licitatorPhone || null,
        licitator_fax: licitatorData.licitatorFax || null,
        licitator_competence: licitatorData.licitatorCompetence || null,
        avatar_url: licitatorData.licitatorAvatar || null,
      };
      ...
      setLicitatorMessage({
        type: 'success',
        text: 'Datele de contact ale licitatorului au fost salvate cu succes! Acestea vor apărea automat în toate anunțurile tale.',
      });
    ...
    const checkAuth = async () => {
      try {
        ...
        const { data: profile } = await supabase
          .from('user_profiles')
          .select('first_name,last_name,email,phone,avatar,licitator_name,licitator_address,licitator_fiscal_code,licitator_consignment_account,licitator_email,licitator_phone,licitator_fax,licitator_competence,avatar_url')
          .eq('user_id', userId)
          .maybeSingle();
        ...
        if (profile) {
          ...
          // Load licitator data
          setLicitatorData({
            licitatorName: profile.licitator_name || '',
            licitatorAddress: profile.licitator_address || '',
            licitatorFiscalCode: profile.licitator_fiscal_code || '',
            licitatorConsignmentAccount: profile.licitator_consignment_account || '',
            licitatorEmail: profile.licitator_email || '',
            licitatorPhone: profile.licitator_phone || '',
            licitatorFax: profile.licitator_fax || '',
            licitatorCompetence: profile.licitator_competence || '',
            licitatorAvatar: profile.avatar_url || ''
          });
        } else {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Verifică dacă oferta este pentru unul dintre produsele noastre
          if (!activeProductIds.includes(newBid.product_id)) {
            return;
          }
          
          // Actualizează productBids pentru produsul respectiv
          setProductBids(prev => {
            const currentBids = prev[newBid.product_id] || [];
            // Verifică dacă oferta nu există deja (pentru a evita duplicate)
            const exists = currentBids.some((b: any) => b.id === newBid.id);
            if (exists) {
              return prev;
            }
      ...
      .subscribe((status) => {
        ...
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          console.warn('[Realtime] Bids: canal indisponibil (publication Realtime pentru `bids`).');
        }
      });
  ...
  const handleEdit = (productId: string) => {
    router.push(`/dashboard/executor/add-auction?id=${productId}`);
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-300 ${
      ...
      {/* Panel Badge */}
      <div className="fixed top-32 right-2 md:top-40 md:right-4 z-50">
        <div className={`inline-flex items-center gap-1.5 md:gap-2 px-2 py-1 md:px-3 md:py-1.5 rounded-lg ${
          ...
          <span className={`text-[10px] md:text-xs font-medium ${
            ...
            Panel privat de executori
          </span>
        </div>
      </div>
      
      
      <div className="w-full max-w-[75%] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-4 md:py-8 flex-1">
        <div className="mb-3 sm:mb-4 md:mb-6">
          <BackButton fallbackHref="/dashboard/executor" label="Înapoi" className="shadow-md" />
        </div>

        <header className="mb-4 sm:mb-6 md:mb-8 flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
          <div className="relative group">
            <div className={`w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full shadow-lg overflow-hidden flex items-center justify-center border-3 sm:border-4 flex-shrink-0 ${
              isDarkMode 
            ...
            <button
              ...
              className={`absolute bottom-0 right-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full text-white shadow-lg flex items-center justify-center transition-all opacity-100 border-2 hover:scale-110 ${
                isDarkMode 
                  ? 'bg-blue-600 hover:bg-blue-700 border-gray-800' 
                  : 'bg-blue-500 hover:bg-blue-600 border-white'
              }`}
              title="Editează avatar"
            >
              <i className="ri-pencil-line text-sm sm:text-base"></i>
            </button>
          </div>
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className={`mt-2 text-sm sm:text-base ${
              ...
              Gestionare licitații publice și execuții judecătorești
            </p>
          </div>
        </header>
        ...
        <div className="mb-3 sm:mb-4 md:mb-6">
          <div className="flex flex-col gap-3 sm:gap-4 mb-3 sm:mb-4">
            ...
            <button
                ...
                className={`w-full sm:w-auto px-3 sm:px-4 md:px-6 py-2 sm:py-2.5 md:py-3 rounded-lg text-xs sm:text-sm md:text-base font-semibold transition-colors flex items-center justify-center gap-1.5 sm:gap-2 text-white shadow-lg ${
                  isDarkMode 
              ...
              >
                <i className="ri-user-settings-line text-base sm:text-lg"></i>
                <span className="hidden sm:inline">Date Contact Licitator</span>
                <span className="sm:hidden">Contact</span>
              </button>
              <button
                onClick={() => setShowManualAddModal(true)}
                className={`w-full sm:w-auto px-2 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-xs sm:text-base transition-colors flex items-center justify-center gap-1 sm:gap-2 shadow-lg ${
                  isDarkMode
              ...
              >
                ...
                <span>Creează anunț manual</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
            ...
            >
              <p className={`text-xs sm:text-sm ${filterStatus === 'all' ? 'text-white/80' : isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total</p>
              <p className={`text-lg sm:text-xl md:text-2xl font-bold ${filterStatus === 'all' ? 'text-white' : isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                {products.length}
              </p>
            </button>
            <button
            ...
            >
              <p className={`text-xs sm:text-sm ${filterStatus === 'active' ? 'text-white/80' : isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Active</p>
              <p className={`text-lg sm:text-xl md:text-2xl font-bold ${filterStatus === 'active' ? 'text-white' : isDarkMode ? 'text-green-400' : 'text-green-600'}`}>
            ...
            >
              <p className={`text-xs sm:text-sm ${filterStatus === 'pending' ? 'text-white/80' : isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>În așteptare</p>
              <p className={`text-lg sm:text-xl md:text-2xl font-bold ${filterStatus === 'pending' ? 'text-white' : isDarkMode ? 'text-orange-400' : 'text-orange-600'}`}>
          ...
          {/* Search */}
          <div className={`p-3 sm:p-4 rounded-lg shadow-sm mb-4 sm:mb-6 ${
            isDarkMode ? 'bg-gray-800' : 'bg-white'
          }`}>
            <input
              type="text"
              ...
              className={`w-full px-3 sm:px-4 py-2 rounded-lg border text-sm sm:text-base focus:outline-none focus:ring-2 ${
                isDarkMode 
        ...
        {isLoading ? (
          ...
          <div className={`text-center py-12 rounded-lg shadow-sm ${
            ...
            {!searchTerm && (
              <button
                onClick={() => router.push('/dashboard/executor/add-auction')}
          ...
          <>
            {/* Desktop Table View */}
            <div className={`hidden md:block rounded-lg shadow-sm overflow-hidden ${
              isDarkMode ? 'bg-gray-800' : 'bg-white'
            }`}>
              ...
              <table className="w-full table-fixed">
                <thead className={isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}>
                  <tr>
                    ...
                    <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider w-[11%] ${
                      isDarkMode ? 'text-gray-300' : 'text-gray-700'
                    }`}>
                      Data
                    </th>
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  {filteredProducts.map((product) => (
                    ...
                    <tr className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-1.5 flex-shrink-0">
                          <button
                            onClick={() => {
                              console.log('[Toggle] Expanding product:', product.id, 'Current state:', expandedProducts[product.id]);
                              toggleProductExpansion(product.id);
                            }}
                            className={`px-3 py-1.5 rounded-lg transition-all duration-200 flex items-center gap-1.5 text-white text-sm font-semibold whitespace-nowrap shadow-md hover:shadow-lg ${
                              isDarkMode 
                            ...
                            title={expandedProducts[product.id] ? 'Ascunde ofertele' : 'Vezi ofertele'}
                          >
                            <i className={`ri-${expandedProducts[product.id] ? 'eye-off' : 'eye'}-line text-sm`}></i>
                            <span>{expandedProducts[product.id] ? 'Ascunde' : 'Vezi'} ofertele</span>
                            {!expandedProducts[product.id] && productBids[product.id] && productBids[product.id].length > 0 && (
                              <span className={`ml-1.5 px-2 py-0.5 rounded-full text-xs font-bold bg-red-500 text-white`}>
                                {productBids[product.id].filter((b: any) => !b.is_winning && b.user_id !== userInfo.supabaseUserId).length}
                              </span>
                            )}
                          </button>
                          {product.status === 'draft' && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className={`px-2 py-1 rounded-lg transition-colors flex items-center gap-1 text-white text-xs whitespace-nowrap ${
                                isDarkMode 
                                  ? 'bg-blue-700 hover:bg-blue-800' 
                                  : 'bg-blue-600 hover:bg-blue-700'
                              }`}
                              title="Preview produs"
                            >
                    ...
                    {expandedProducts[product.id] && (
                      <tr>
                        <td colSpan={6} className={`px-6 py-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
          ...
          <div className="md:hidden space-y-2 sm:space-y-3">
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  {/* Header produs - Compact pe mobil */}
                    ...
                    <div className="flex flex-col sm:flex-row items-start gap-1.5 sm:gap-2">
                      ...
                      <div className="flex-1 min-w-0 w-full sm:w-auto">
                        <div className="flex items-start justify-between gap-1.5 sm:gap-2 mb-1">
                          ...
                          {winningBids.length > 0 && (
                            <span className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-semibold flex items-center gap-1 sm:gap-1.5 whitespace-nowrap flex-shrink-0 ${
                              isDarkMode
                        ...
                        <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                          <span className="font-medium">Ofertă maximă: </span>
                          {formatPrice(highestBid, product.currency)}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 mt-1.5 sm:mt-2">
                          <button
                            ...
                            className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg transition-all duration-200 flex items-center gap-1 text-white text-xs font-semibold whitespace-nowrap shadow-md hover:shadow-lg ${
                              isDarkMode 
                                ? expandedProducts[product.id]
                                  ? 'bg-gray-600 hover:bg-gray-700'
                                  : 'bg-gradient-to-r from-green-600 to-green-700 hover:from-green-500 hover:to-green-600'
                            ...
                            title={expandedProducts[product.id] ? 'Ascunde ofertele' : 'Vezi ofertele'}
                          >
                            <i className={`ri-${expandedProducts[product.id] ? 'eye-off' : 'eye'}-line text-xs`}></i>
                            <span>{expandedProducts[product.id] ? 'Ascunde' : 'Vezi'} ofertele</span>
                            {!expandedProducts[product.id] && bids.length > 0 && (
                              <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-xs font-bold bg-red-500 text-white`}>
                                {bids.filter((b: any) => !b.is_winning && b.user_id !== userInfo.supabaseUserId).length}
                              </span>
                            )}
                          </button>
                          <button
                            onClick={() => handleEdit(product.id)}
                            className={`px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-lg transition-colors text-white text-xs whitespace-nowrap ${
                              isDarkMode 
                                ? 'bg-blue-600 hover:bg-blue-700' 
                                : 'bg-blue-500 hover:bg-blue-600'
                            }`}
                          ...
                          <button
                            onClick={() => handleDelete(product.id, product.title)}
                            className={`px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-lg transition-colors text-white whitespace-nowrap ${
                              isDarkMode 
                                ? 'bg-red-600 hover:bg-red-700' 
                                : 'bg-red-500 hover:bg-red-600'
                            }`}
        ...
        {/* Import Modal */}
        {showImportModal && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-2 md:p-4">
            <div className={`w-full sm:w-full md:max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-lg md:rounded-xl shadow-2xl ${
              isDarkMode ? 'bg-gray-800' : 'bg-white'
            }`}>
                ...
                <div className="flex flex-col items-center mb-6">
                  <div className={`w-32 h-32 rounded-full shadow-lg overflow-hidden flex items-center justify-center border-4 mb-4 ${
                    ...
                    {avatarPreview ? (
                      ...
                      <img src={userInfo.avatar} alt="Current Avatar" className="w-full h-full object-cover" />
                    ) : (
        ...
        {showLicitatorModal && (
          ...
          >
            <div 
              className={`w-full max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl ${
                isDarkMode 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <div className={`sticky top-0 z-10 flex items-center justify-between p-4 sm:p-5 md:p-6 border-b ${
                ...
                <div className="flex-1 min-w-0 pr-2">
                  ...
                  <p className={`mt-1.5 text-sm sm:text-base ${
                    ...
                    Completează datele de contact ale licitatorului care vor apărea în toate anunțurile tale de licitații publice. Nu este nevoie să le completezi pentru fiecare produs.
                  </p>
                </div>
                <button
                  onClick={() => {
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={licitatorData.licitatorFiscalCode}
                        onChange={(e) => setLicitatorData(prev => ({ ...prev, licitatorFiscalCode: e.target.value }))}
                        className={`w-full px-4 py-3 rounded-lg border transition-all duration-300 ${
                          isDarkMode
                            ? 'bg-gray-700/50 border-gray-600 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                            : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={licitatorData.licitatorConsignmentAccount}
                        onChange={(e) => setLicitatorData(prev => ({ ...prev, licitatorConsignmentAccount: e.target.value }))}
                        className={`w-full px-4 py-3 rounded-lg border transition-all duration-300 ${
                          isDarkMode
                            ? 'bg-gray-700/50 border-gray-600 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                            : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                        }`}
                        placeholder="Cont de consemnare"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="tel"
                        value={licitatorData.licitatorFax}
                        onChange={(e) => setLicitatorData(prev => ({ ...prev, licitatorFax: e.target.value }))}
                        className={`w-full px-4 py-3 rounded-lg border transition-all duration-300 ${
                          isDarkMode
                            ? 'bg-gray-700/50 border-gray-600 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                            : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start">
                      ...
                      <div className="flex-shrink-0">
                        {licitatorAvatarPreview || licitatorData.licitatorAvatar ? (
                          <div className="relative w-32 h-32 rounded-full overflow-hidden ring-2 ring-blue-500">
                            <img
                              src={licitatorAvatarPreview || licitatorData.licitatorAvatar || ''}
                              alt="Avatar preview"
        ...
        {showManualAddModal && (
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg">
              <h2>Modal Test</h2>
              <button onClick={() => setShowManualAddModal(false)}>Close</button>
            </div>
          </div>
        )}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      const { data: sessionData } = await supabase.auth.getSession();
                      if (!sessionData.session) {
                        showNotification('error', 'Eroare', 'Trebuie să fii autentificat');
                        return;
                      }
                      ...
                      if (response.ok) {
                        ...
                        const result = await response.json();
                        showNotification('error', 'Eroare', result.error || 'Eroare la trimiterea contraofertei');
                      }
                    } catch (error: any) {
      ...
      {showNotificationModal && notificationModal && (
        ...
        {showQuickAddModal && quickAddIsGenerating && (
          <div 
            ...
            >
              ...
              <div className="mb-6">
                <div className={`h-3 rounded-full overflow-hidden ${
                  ...
                  <div 
                    className="h-full bg-gradient-to-r from-blue-500 via-blue-500 to-pink-500 rounded-full transition-all duration-300 ease-out relative overflow-hidden"
            ...
            >
              <div className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-inherit">
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  <i className="ri-flashlight-line mr-2 text-blue-500"></i>
                  Creează cu GoBid AI
                </h2>
                <button
                ...
                >
                  <span className="text-sm font-medium">Ai nevoie de ajutor</span>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="relative">
                  ...
                  <div className="border-2 border-dashed rounded-lg p-6 text-center transition-colors hover:border-blue-500"
                    ...
                    <input
                      ...
                      id="quick-add-images"
                    />
                    <label htmlFor="quick-add-images" className="cursor-pointer">
                      <i className="ri-image-add-line text-4xl mb-2 text-gray-400"></i>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                        Click pentru a încărca imagini sau trage-le aici
                      </p>
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    placeholder="Descrie produsul tău... (ex: Telefon iPhone 13, stare excelentă, folosit 1 an, fără zgârieturi)"
                    rows={6}
                ...
                <div className="relative">
                  <label className={`block text-sm font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    Oraș <span className="text-xs font-normal text-gray-500">(completat automat din profil)</span>
                  </label>
                  {showHelpModal && helpStep === 2 && (
                    <div className={`absolute left-0 right-0 top-full mt-2 p-3 rounded-lg border-2 border-blue-500 bg-blue-50 shadow-lg z-10 ${isDarkMode ? 'bg-blue-900/30 border-blue-400' : 'bg-blue-50 border-blue-500'}`}>
                      <div className="flex items-start gap-2">
                        <i className="ri-information-line text-blue-500 text-lg mt-0.5"></i>
                        <p className={`text-sm flex-1 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                ...
                {/* Currency Selector and Price on same row */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      <label className={`flex items-center cursor-pointer ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        <input
                          type="radio"
                      ...
                      <label className={`flex items-center cursor-pointer gap-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        <input
                          type="radio"
                          name="quickAddCurrency"
                          value="EUR"
                          ...
                          className="mr-0"
                        />
                        <i className="ri-euro-line text-lg"></i>
                        <span>EUR</span>
                      </label>
                    </div>
                  </div>
                ...
                {quickAddGeneratedProduct && (
                  <div className={`border-2 rounded-lg p-6 ${isDarkMode ? 'border-blue-500 bg-blue-900/10' : 'border-blue-500 bg-blue-50'}`}>
                    ...
                    <div className="space-y-4">
                      {/* Editable Title */}
                      <div>
                        <label className={`block text-sm font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Titlu</label>
                        <input
                          type="text"
                          value={editableTitle}
