...
export default function PieseAutoSupportPage() {
  const router = useRouter();
  const { isDarkMode } = usePieseAutoTheme();

  useLayoutEffect(() => {
    router.replace("/dashboard/support?context=piese-auto");
  }, [router]);

  return (
    <div className={`rounded-xl p-6 ${isDarkMode ? "bg-gray-800/80 border border-gray-700" : "bg-white border border-gray-200"}`}>
      <p className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
        Se redirecționează la Suport…
      </p>
      <Link
        href="/dashboard/support?context=piese-auto"
