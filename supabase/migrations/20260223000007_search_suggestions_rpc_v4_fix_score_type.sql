-- ============================================
-- Migration v4: fix type mismatch score (double precision → numeric)
-- Comportament identic cu v3; doar cast explicit la numeric pentru score.
-- ============================================
...
BEGIN
  ...
  FROM (
    SELECT DISTINCT ON (combined.dedupe_key)
      ...
      combined.score
    FROM (
      -- Sugestii principale: dedupe_key = phrase_norm, score numeric
      SELECT
        s.phrase,
        s.kind,
        s.popularity,
        ...
        (
          (CASE WHEN s.phrase_norm LIKE q_trim || '%' THEN 10 ELSE 0 END)::numeric
          + (similarity(s.phrase_norm, q_trim)::numeric * 5::numeric)
      ...
      -- Sinonime: doar dacă q_norm >= 2 caractere; max 20 candidate; dedupe_key = to_norm, score numeric
      SELECT
...
COMMENT ON FUNCTION public.search_suggestions_rpc(text, text, int) IS
  'v4: score cast explicit la numeric (fix 42804). Comportament v3.';
