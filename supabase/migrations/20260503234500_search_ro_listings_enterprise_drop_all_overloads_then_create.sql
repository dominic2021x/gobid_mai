-- Fix duplicate/stale search_ro_listings_enterprise overloads:
-- Prior migration used DROP FUNCTION IF EXISTS with signatures that did not match remote
-- (NOTICE: does not exist, skipping). CREATE OR REPLACE then added a NEW overload while old
-- signatures could remain. This migration drops ALL public overloads, then recreates the canonical RPC.

DO $drop_all_search_ro_listings_enterprise$
DECLARE
  fq text;
BEGIN
  FOR fq IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'search_ro_listings_enterprise'
      AND p.prokind = 'f'
  LOOP
    EXECUTE 'DROP FUNCTION ' || fq;
  END LOOP;
END
$drop_all_search_ro_listings_enterprise$;
